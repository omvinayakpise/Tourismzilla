...
<?php
    ...
    if (isset($_POST['continue'])) {
            ...
            $query = "INSERT INTO `flightbooking2` values ('$first_name', '$last_name', '$gender', '$mobile_no', '$email_id', '$date_of_birth', '$date_of_journey', '  $from_loc', '$to_loc', '$no_of_travellers', '$class');";
            $result = mysqli_query($conn, $query) or die(mysqli_error($conn));
            if($result) {
                $message = "Successfully registered!!!";
                echo "<script type='text/javascript'>
                alert('$message');
                  window.location.replace('flight4.php');
                </script>";
